...
├── config/                   # 애플리케이션 및 모델 설정 파일 (Hydra 기반)
│   ├── dataset/              # 데이터셋 관련 설정
│   ├── model/                # 모델 관련 설정
│   ├── training/             # 훈련 관련 설정
│   ├── utils/                # 유틸리티 관련 설정
│   ├── config.yaml           # 주요 설정 파일
│   └── config_schema.py      # 설정 스키마 정의 (유효성 검사용)
├── data/                     # 데이터 저장 디렉토리
│   ├── processed/            # 전처리된 데이터
│   └── raw/                  # 원본 데이터
...
### 주요 모듈 설명 (`src/my_template/`)

* **`abc_interfaces/`**: 프로젝트의 주요 컴포넌트(데이터, 모델, 훈련, 유틸리티)에 대한 **추상 기본 클래스(ABC) 인터페이스**를 정의합니다. 이를 통해 각 모듈이 구현해야 할 필수 메서드들을 명확히 제시하여 코드의 일관성과 확장성을 보장합니다.
* **`data/`**: 데이터 로딩, 전처리, 데이터셋 구성 및 시각화와 관련된 모듈들을 포함합니다. (`DataLoader.py`, `DataPreprocessor.py`, `Dataset.py`, `DataVisualizer.py` 등)
* **`model/`**: 프로젝트에서 사용할 인공지능 **모델의 구현**을 담는 모듈입니다. (`Model.py`에서 모델 아키텍처를 정의)
* **`training/`**: 모델 학습 프로세스와 관련된 모든 모듈을 포함합니다. 콜백, 손실 함수, 옵티마이저, 트레이너, 실험 관리자 등 **학습 파이프라인을 구성하는 핵심 요소**들이 있습니다.
* **`utils/`**: 로깅, 설정 관리, 모델 저장 등 프로젝트 전반에 걸쳐 사용되는 **유틸리티성 모듈**들을 제공합니다. (`ConfigurationManager.py`, `Logger.py`, `Saver.py` 등)
...
아직 시스템에 Poetry가 설치되어 있지 않다면, 다음 두 가지 방법 중 하나를 선택하여 설치하세요. **`pipx`를 통한 설치를 강력히 권장합니다.**

#### 1. `pipx`를 통한 설치 (권장)

`pipx`는 Python 애플리케이션을 격리된 환경에 설치하고 관리하는 도구입니다. 시스템 Python 환경을 오염시키지 않으면서 Poetry를 안전하게 설치할 수 있습니다.

먼저 `pipx`가 설치되어 있지 않다면 설치합니다:

```bash
pip install pipx
pipx ensurepath # pipx 실행 경로를 PATH에 추가
```

그다음 `pipx`를 사용하여 Poetry를 설치합니다:
...
`pipx`를 사용할 수 없거나 다른 방법으로 설치하고 싶다면, Poetry 공식 설치 스크립트를 사용할 수 있습니다.
...
이제 `git commit` 명령을 실행할 때마다 **`.pre-commit-config.yaml`**에 정의된 훅들이 자동으로 실행됩니다. `ruff`를 이용한 코드 린트 검사 및 포매팅이 자동으로 이루어지며, 코드가 자동으로 수정되거나 린트 오류가 발견되면 커밋이 중단될 수 있습니다. 이때는 수정된 파일을 다시 `git add .` 한 후 커밋을 재시도하면 됩니다.
...
* **테스트 스크립트 실행 (대안):**
    ...
    ```
    이 스크립트도 내부적으로 `pytest`를 호출하여 모든 테스트를 실행합니다.
...
`config/` 폴더에는 프로젝트의 다양한 설정 파일들이 `.yaml` 형식으로 정의되어 있습니다. 이 파일들은 **모델의 하이퍼파라미터, 데이터 경로, 훈련 파라미터 등** 모델 내부의 다양한 객체들(예: Logger, Model, Dataset)을 설정하는 데 사용됩니다.

* **`config/dataset/base.yaml`**: 데이터셋 관련 설정
* **`config/model/base.yaml`**: 모델 관련 설정
* **`config/training/base.yaml`**: 훈련 관련 설정
* **`config/utils/base.yaml`**: 유틸리티 관련 설정

대부분의 설정은 이 파일들을 통해 관리되므로, 특정 파라미터를 변경하려면 해당 `.yaml` 파일을 수정하면 됩니다.

---

## 📊 데이터

프로젝트에서 사용되는 **데이터셋은 자유롭게 선택하여 사용**할 수 있습니다.

**가공되지 않은 원본 데이터**는 `data/raw/` 폴더에 배치해야 합니다. 데이터 전처리 스크립트는 이 `raw` 폴더의 데이터를 읽어 `data/processed/` 폴더에 저장하게 됩니다.
...
* **`src/my_template/` 내부 구현**:
    * `src/my_template/utils/` 폴더를 제외한 모든 모듈 (예: `data`, `model`, `training`, `abc_interfaces`)에서 **미리 정의된 인터페이스 및 메소드**를 구현합니다. 각 파일은 해당 도메인의 핵심 로직을 담도록 설계되어 있습니다.
